...
# --- Параметры датасета -----------------------------------------------------
# Количество примеров на категорию по умолчанию. По ТЗ минимально достаточно
# 10, но для устойчивого обобщения (корректное распознавание коротких явных
# запросов вроде "создание сайта") используется больше. Скрипт
# ``scripts/generate_data.py`` позволяет переопределить значение через флаг
# ``--per-category``.
EXAMPLES_PER_CATEGORY: int = 30
RANDOM_SEED: int = 42
