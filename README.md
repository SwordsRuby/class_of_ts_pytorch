...
1. **Генерация данных** — синтетический датасет из 150 текстов (30 на категорию).
2. **Предобработка текста** — нижний регистр, удаление стоп-слов, токенизация
   и стемминг словоформ (Snowball, русский язык).
...
Создаёт `data/raw/requests_dataset.csv` со 150 примерами (30 на категорию).
По ТЗ достаточно 10 на категорию, но для устойчивого распознавания
коротких запросов (например, «создание сайта») используется больше.
Значение можно изменить флагом `--per-category`.
...
- Стемминг (нормализация словоформ русского языка) выполняется в
  `src/preprocessing.py` — позволяет распознавать разные падежи/числа слов
  («сайт», «сайта», «сайтов»).
